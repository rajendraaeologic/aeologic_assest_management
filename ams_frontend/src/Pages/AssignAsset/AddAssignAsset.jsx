  ...
  import {createAssignAsset, getAllAssignAssets} from "../../Features/slices/assignAssetSlice.js";

  const AddAssignAsset = ({ onClose,onSuccess }) => {
  ...
  // User handlers
    ...
    useEffect(() => {
      register("branchId", {
        required: assignAssetStrings?.assignAsset?.validation?.branchRequired || "Branch is required",
      });
      register("departmentId", {
        required: assignAssetStrings?.assignAsset?.validation?.departmentRequired || "Department is required",
      });
    }, [register]);
    ...
    const onSubmit = async (data) => {
      ...
      try {
        ...
        // Handle other errors
        toast.error(errorMessage || assignAssetStrings.updateAssignAsset.toast.error, {
          position: "top-right",
          autoClose: 5000,
        });
      }
                  ...
                  <div className="w-full relative">
                    ...
                    >
                      {assets.find(a => a.id === assetId)
                          ? `${assets.find(a => a.id === assetId).assetName} (${assets.find(a => a.id === assetId).uniqueId})`
                          : assignAssetStrings.updateAssignAsset.select.assetDefault}
                    </div>
                    {errors.assetId && (
                        <p className="text-red-500 text-sm mt-1">
                          ...
                          >
                            {assets.map((asset) => (
                                <li
                                    ...
                                    className="px-4 py-2 hover:bg-gray-100 cursor-pointer"
                                >
                                  {asset.assetName} ({asset.uniqueId}) - {asset.branch?.branchName}
                                </li>
                            ))}
                            {loadingAssets && (
                                <li className="px-4 py-2 text-sm text-gray-500">
