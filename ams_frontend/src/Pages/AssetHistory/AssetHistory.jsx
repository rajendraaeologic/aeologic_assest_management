...
import {
    FiClock, FiUser, FiLayers, FiTag, FiSettings, FiAlertCircle,
    FiCalendar, FiActivity, FiHardDrive, FiInfo, FiChevronRight
} from "react-icons/fi";
import { CiSaveUp2 } from "react-icons/ci";
import { MdExpandMore, MdExpandLess } from "react-icons/md";

const statusConfig = {
    ACTIVE: { color: "bg-green-500", icon: <FiActivity className="text-green-600" /> },
    IN_ACTIVE: { color: "bg-gray-500", icon: <FiAlertCircle className="text-gray-600" /> },
    IN_USE: { color: "bg-blue-500", icon: <FiHardDrive className="text-blue-600" /> },
    UNDER_MAINTENANCE: { color: "bg-yellow-500", icon: <FiSettings className="text-yellow-600" /> },
    RETIRED: { color: "bg-red-500", icon: <FiAlertCircle className="text-red-600" /> },
    ASSIGNED: { color: "bg-indigo-500", icon: <FiUser className="text-indigo-600" /> },
    UNASSIGNED: { color: "bg-purple-500", icon: <FiUser className="text-purple-600" /> },
    CREATED: { color: "bg-teal-500", icon: <FiLayers className="text-teal-600" /> },
    UPDATED: { color: "bg-cyan-500", icon: <FiSettings className="text-cyan-600" /> },
    DELETED: { color: "bg-rose-500", icon: <FiAlertCircle className="text-rose-600" /> },
...
const TimelineEvent = ({ event, isLast, index }) => {
    const config = statusConfig[event.action] || statusConfig[event.status] || statusConfig.DEFAULT;
    ...
    return (
        <motion.div
            ...
            whileHover={{ scale: 1.02, backgroundColor: "rgba(239, 246, 255, 0.7)", transition: { duration: 0.2 } }}
            className="relative pb-6 pl-3 pr-3 rounded-lg cursor-pointer group"
        >
            <div className={`absolute left-[-26px] top-1 h-5 w-5 rounded-full ${config.color} flex items-center justify-center z-10`}>
                <div className="text-white text-xs">{React.cloneElement(config.icon, { className: "h-3 w-3" })}</div>
            </div>
              ...
              {React.cloneElement(config.icon, { className: "mr-2" })}
                ...
                {event.user && (
                    ...
                    >
                        ...
                        <div>
                            <span className="text-sm text-gray-600 block">{event.user.userName}</span>
    ...
    <div className="relative group inline-block">
        ...
        >
            <div className="space-y-3">
                ...
                <DetailItem label="Asset Name " value={asset.assetName} icon={<FiTag className="text-gray-500" />} />
                <DetailItem label="Unique ID" value={asset.uniqueId} icon={<FiLayers className="text-gray-500" />} />
                <DetailItem label="Model" value={asset.model || "N/A"} icon={<FiHardDrive className="text-gray-500" />} />
                <DetailItem label="Serial Number" value={asset.serialNumber || "N/A"} icon={<FiSettings className="text-gray-500" />} />
                <DetailItem label="Brand" value={asset.brand || "N/A"} icon={<FiActivity className="text-gray-500" />} />
                <DetailItem
                    label="Status"
                    value={
              ...
              {statusConfig[asset.status]?.icon && React.cloneElement(statusConfig[asset.status].icon, { className: "mr-1 h-3 w-3" })}
                            {asset.status}
            </span>
                    }
...
const AssetHistory = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (response.data?.success) {
                    ...
                    if (groupedHistories.length === 0) setError(strings.noHistoryAvailable);
                } else throw new Error(response.data?.message || "Failed to fetch data");
            } catch (err) {
    ...
    const groupByAsset = (histories) => {
        ...
        histories.forEach(history => {
            ...
            if (!grouped[history.assetId]) grouped[history.assetId] = { asset: history.asset, histories: [] };
            grouped[history.assetId].histories.push(history);
        });
        Object.keys(grouped).forEach(assetId => {
            grouped[assetId].histories.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
        });
    ...
    const getLatestAction = (latestHistory) => {
        ...
        switch(latestHistory.action) {
            case "ASSIGNED": return `Assigned to ${latestHistory.user?.userName || "user"}`;
            case "UNASSIGNED": return "Unassigned from previous user";
            case "STATUS_CHANGE": return `Status changed to ${latestHistory.status?.toLowerCase() || "new status"}`;
            case "CREATED": return "Asset created";
            case "UPDATED": return "Asset details updated";
            case "DELETED": return "Asset deleted";
            default: return latestHistory.action.toLowerCase();
        }
    };
    ...
    if (error || histories.length === 0) return (
        <div className={`w-full min-h-screen bg-slate-100 px-2 ${isSidebarOpen ? "overflow-hidden" : "overflow-y-auto overflow-x-hidden"}`}>
            <div className={`mx-auto min-h-screen ${isSidebarOpen ? "pl-0 md:pl-[250px] lg:pl-[250px]" : "pl-0 md:pl-[90px] lg:pl-[90px]"}`}>
                <div className="pt-24">
                    <div className="flex justify-between mx-5 mt-2">
                        <h3 className="text-xl font-semibold text-[#6c757D]">{strings.title}</h3>
                    ...
                    <div className="mx-5 flex gap-2 mb-4 items-center">
                        <button onClick={handleNavigate} className="text-[#6c757D] hover:text-blue-600 transition-colors flex items-center">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-center min-h-[50vh]">
                    <div className="text-center p-8 bg-white rounded-lg shadow-md max-w-md mx-auto">
                        ...
                        <h3 className="text-lg font-medium text-gray-800 mb-2">{error || strings.noAssetsFound}</h3>
                        <p className="text-gray-600">
                            {error === strings.noAssetsFound
                                ? "There are no assets available to show history for."
                                : "Please check back later or contact support if you believe this is an error."}
                        </p>
                        <button onClick={handleNavigate} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                            Return to Dashboard
                        </button>
                    </div>
                </motion.div>
    ...
    return (
        <div className={`w-full min-h-screen bg-slate-100 px-2 ${isSidebarOpen ? "overflow-hidden" : "overflow-y-auto overflow-x-hidden"}`}>
            <div className={`mx-auto min-h-screen ${isSidebarOpen ? "pl-0 md:pl-[250px] lg:pl-[250px]" : "pl-0 md:pl-[90px] lg:pl-[90px]"}`}>
                <div className="pt-24">
                    <div className="flex justify-between mx-5 mt-2">
                        <motion.h3 initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="text-xl font-semibold text-[#6c757D] flex items-center">
                                ...
                                >
                                    <div className="mb-4">
                                        ...
                                        <div className="flex items-center mt-2 text-gray-500">
                                            ...
                                            <span className="ml-1 capitalize font-medium">{asset.status.toUpperCase()}</span>
                                        </div>
                                        {histories.length > 0 && (
                                            <div className="flex items-center mt-2 text-gray-500">
                                                <FiClock className="mr-2" />
                                                <span>Last Action: </span>
                                                <span className="ml-1 font-medium">{getLatestAction(histories[0])}</span>
                                            </div>
                                        )}
                                        {asset.assignedUser && (
                                            <div className="flex items-center mt-2 text-gray-500">
                                                ...
                                                <span className="ml-1 font-medium">{asset.assignedUser.userName || asset.assignedUser.email}</span>
                                            </div>
                                        )}
                                    </div>
                                    <motion.button
                                    ...
                                    >
                                        {expandedIds.includes(asset.id) ? (
                                            <><MdExpandLess className="mr-1" />{strings.hideDetails}</>
                                        ) : (
                                            <><MdExpandMore className="mr-1" />{strings.showDetails}</>
                                        )}
                                    </motion.button>
                                    <AnimatePresence>
                                        {expandedIds.includes(asset.id) && (
                                                    ...
                                                    <div className="space-y-6">
                                                        <div>
                                                            ...
                                                            {asset.company?.organizationName && (
                                                                <DetailItem label="Organization" value={asset.company.organizationName} icon={<FiLayers />} />
                                                            )}
                                                            {asset.branch?.branchName && (
                                                                <DetailItem label="Branch" value={asset.branch.branchName} icon={<FiSettings />} />
                                                            )}
                                                            {asset.department?.departmentName && (
                                                                <DetailItem label="Department" value={asset.department.departmentName} icon={<FiHardDrive />} />
                                                            )}
                                                        </div>
                                                        <div>
                                                            <h3 className="font-semibold text-gray-800 mb-3 flex items-center">
                                                                ...
                                                                {histories.length > 0 ? (
                                                                    <>
                                                                        {histories.map((history, index) => (
                                                                            <TimelineEvent key={history.id} event={history} isLast={index === histories.length - 1} index={index} />
                                                                        ))}
                                                                    ...
                                                                    <p className="text-gray-500 text-sm italic">No history records available for this asset</p>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </div>
