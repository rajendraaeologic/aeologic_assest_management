...
export const getAssetAssignmentByIdService = async (assignmentId) => {
  const response = await API.get(`/assignAsset/asset-assignments/${assignmentId}`);
  return response.data;
};

export const getAssetAssignmentsByAssetIdService = async (assetId) => {
  const response = await API.get(`/assignAsset/asset-assignments/asset/${assetId}`);
  return response.data;
};

export const getAssetAssignmentsByUserIdService = async (userId) => {
  const response = await API.get(`/assignAsset/asset-assignments/user/${userId}`);
  return response.data;
};

export const updateAssignAssetService = async (data) => {
  const response = await API.put(
      `/assignAsset/asset-assignments/${data.assignmentId}`,
      data.body
  );
  return response.data;
};
...
export const deleteAssignAssetService = async (ids) => {
  try {
    ...
    if (idsArray.length === 1) {
      const response = await API.delete(`/assignAsset/asset-assignments/${idsArray[0]}`);
      return {
        deletedAssignAssetIds: [idsArray[0]],
        success: true,
      };
    } else {
      const response = await API.post("/assignAsset/asset-assignments/bulk-delete", {
