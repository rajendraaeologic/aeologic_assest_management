...
const getUserNotifications = catchAsync(async (req, res) => {
    ...
    const notifications = await userNotificationService.getUserNotifications(user.id, filter, options);
    res.send(notifications);
});

const markUserNotificationAsRead = catchAsync(async (req, res) => {
    const user = req.user as User;
    if (!req.body.notificationIds || !(req.body.notificationIds instanceof Array) || !req.body.notificationIds.length) {
        throw new ApiError(httpStatus.NOT_FOUND, 'No notification found');
    }

    const notifications = await userNotificationService.markUserNotificationAsRead(user.id, req.body.notificationIds);
