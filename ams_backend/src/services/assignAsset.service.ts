...
const getAvailableAssets = async (branchId?: string, departmentId?: string): Promise<Asset[]> => {
...
// Unassign asset
const unassignAsset = async (assignmentId: string): Promise<{
    assignment: any;
    asset: Asset;
}> => {
...
const getAssetAssignments = async (
    ...
    const { limit = 10, page = 1, sortBy = "assignedAt", sortType = "desc" } = options;
...
export const getUsersByDepartmentId = async (
    ...
    const searchConditions: Prisma.UserWhereInput = options.searchTerm?.trim()
        ? {
            OR: [
                { userName: { contains: options.searchTerm, mode: "insensitive" as const } },
                { email: { contains: options.searchTerm, mode: "insensitive" as const } },
            ],
        }
        : {};
...
const updateAssetAssignment = async (
    ...
    // Start transaction
    const [updatedAssignment, updatedAsset, oldAsset] = await db.$transaction(async (prisma) => {
        ...
        const updatedAsset = updateData.assetId && updateData.assetId !== existingAssignment.assetId
            ? await prisma.asset.update({
                where: { id: updateData.assetId },
                data: {
                    status: AssetStatus.IN_USE,
                    assignedToUserId: updateData.userId || existingAssignment.userId,
                },
            })
            : existingAssignment.asset;

        // Update the old asset (if asset was changed)
        const oldAsset = updateData.assetId && updateData.assetId !== existingAssignment.assetId
            ? await prisma.asset.update({
...
const deleteAssignmentsByIds = async (
    ...
    try {
        ...
        throw new ApiError(
            httpStatus.INTERNAL_SERVER_ERROR,
            "Failed to delete assignments. Please try again later."
        );
    }
};
